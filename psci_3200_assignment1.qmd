...
ggplot(eth_svy, aes(x = z_pol_base, fill = "Baseline")) +
  geom_density(alpha = 0.5) +
  geom_density(data = eth_svy, aes(x = z_pol_end, fill = "Endline"), alpha = 0.5) +
  labs(y = "Percent of respondents", x = NULL, 
       title = "Comparison of Feelings of Political Efficacy") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Baseline" = palette[9], "Endline" = palette[5])) +
  theme_minimal()
                 

ggplot(eth_svy, aes(x = z_future_base, fill = "Baseline")) +
  geom_density(alpha = 0.5) +
  geom_density(data = eth_svy, aes(x = z_future_end, fill = "Endline"), alpha = 0.5) +
  labs(y = "Percent of respondents", x = NULL, 
       title = "Comparison of Public Sector or Civil Society Career Plans") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Baseline" = palette[9], "Endline" = palette[5])) +
  theme_minimal()
...
- Given that this survey data contains two waves, the baseline graphs represent student responses collected in May-June whereas the endline graphs represent student responses collected in October-November of 2022. 

- In words, describe whether the figures tell us anything about changes over time.

- The data is standarized so you can't see changes in the mean or standard deviation over time. You would only be able to see skew and peaks but that doesn't really change in this case. 
...
- Continuous outcome with two binary predictors
...
- Interpreting the coefficients from the model:

Intercept (-0.108): This represents the estimated baseline outcome (future plans index) for individuals who are female (gender_baseMale = 0) and have "Feelings of political efficacy" below the sample mean.

bi_pol_base (0.111): This represents the estimated change in the outcome (future plans index) for individuals with "Feelings of political efficacy" above the sample mean compared to those below the mean, but only for females (gender_baseMale = 0). However, the p-value (0.066) indicates that this coefficient is not statistically significant at conventional levels.

- The sample mean in this case is zero because its standardized.

gender_baseMale (0.068): This represents the estimated difference in the outcome (future plans index) between males (gender_baseMale = 1) and females (gender_baseMale = 0) for individuals with "Feelings of political efficacy" below the sample mean.

bi_pol_base × gender_baseMale (-0.011): This represents the difference in the effect of "Feelings of political efficacy" on the outcome (future plans index) between males and females. A negative coefficient indicates that the effect of "Feelings of political efficacy" on future plans is slightly weaker for males compared to females, but this interaction effect is not statistically significant according to the p-value (0.077).

- gender does not have and impact on the relationship between feelings of effiacy and the outcome. 

Substantive Interpretation:

The interactive relationship described in the data suggests that the effect of "Feelings of political efficacy" on future plans may vary by gender, but the evidence for this interaction effect is not strong. While individuals with higher "Feelings of political efficacy" tend to have slightly more positive future plans, this effect does not significantly differ between males and females in this dataset.
...
- In the original model, the regression coefficient represents the increase in the future plans index for a one-unit increase (or an increase of one standard deviation) in "Feelings of political efficacy" index, holding all other variables constant. In this model, the regression coefficent represents the change in the "Future plans in Public Sector or Civil Society" index variable (future) associated with a one-unit increase in "Feelings of political efficacy" index variable (pol), while accounting for within-individual variation over time. Specifically, for each individual, a one-unit increase (or increase of one standard deviation) in feelings of political efficacy is associated with an average increase of 0.016 units in the future plans in the public sector or civil service average z-score index, holding constant individual-specific effects that remain constant over time.
