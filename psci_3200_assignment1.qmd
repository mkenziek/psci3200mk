...
# did you include poloffice and ngo in this part?
...
z_score = function(x, y){
  ...
  mean = mean( as.numeric( unlist(x, y)) , na.rm = T)
  sd = sd( as.numeric( unlist(x, y)) , na.rm = T)
  # subtract mean; divide by SD
  ( as.numeric(x[, y, drop = TRUE]) - mean) / sd
}

# calculate z-scores using for loop
...
- This data contains two waves collected in May-June and October-November of 2022. A total of 825 students completed both waves of the survey. This data was part of a randomized experiment inviting students to participate in a one-day workshop that connected them with opportunities to interact with leaders of civil society, sign-up as volunteers in civil society organizations, and engage in structured dialogue about politics with a diverse group of peers.

-  If your z-score is 0, it means you scored exactly average. A positive z-score means you scored above average, and a negative z-score means you scored below average. The further away from 0 your z-score is, the more different your score is from the average.
...
bi_pol_model <- list()
bi_pol_model[['Multivariate']] = lm(z_future_base ~ z_pol_base + bi_pol_base*gender_base, eth_model)
