...
-   Link: <https://www.aeaweb.org/articles?id=10.1257/aer.20121637>

## My research Question

-   Independent Variables:

    1.  The margin of victory for the PAN (National Action Party) candidate.
    2.  Create an ordinal index of municipal alternation, municipal alternation index, which identifies how many times municipalities experienced a rotation in municipal powers. Municipalities with no alternation in municipal power receive a zero score. Municipalities with an alteration in party from one election to the next get one point for each time they alternate.
        -   My election data ranges from 2001-2020.
    3.  Create an index of PAN power stability, PAN power index , which identifies how many mayoral terms the PAN party has stayed in office in a municipality. Each PAN term between where the PAN was in power represents a score of one. This will be an additive index.

-   Dependent Variables:

    1.  Post-election municipal homicide rate per 100,000 people (includes everyone, not just inter-cartel murders).
...
### Figure 2: PAN-PRI Close Elections Overall
...
### Figure 3: All Party Homicide Rate by Party Alternation Index Boxplot

```{r, warning = FALSE, fig.width=10, fig.height= 15}

# Set the outer margin to allow more space for the main title
par(mar = c(5, 6, 4, 2) + 0.1)

# Create the boxplot with adjusted margins
boxplot(post_election_avg_rate ~ alternation_index,
        data = elections_hom,
        xlab = "Municipal Level Political Party Alternations",
        ylab = "Average Post-Election Homicide Rate (Per 100,000 People)",
        main = "Figure 3. Post-Election Municipal Homicide Rates in Mexico (2001-2020) by Political Party Alternations") 


```

### Figure 4: All Party Homicide Rate by PAN Power Index Boxplot
...
boxplot(post_election_avg_rate ~ pan_consec_yrs_in_pwr,
        data = elections_hom,
        xlab = "Municipal Level PAN Power Index",
        ylab = "Average Post-Election Homicide Rate (Per 100,000 People)",
        main = "Post-Election Municipal Homicide Rates in Mexico (2001-2020) by PAN Power Stability") 

```

### Figure 5: Party Alternation Index Histogram
...
ggplot(elections_hom, aes(x = post_election_avg_rate)) +
  ...
  theme_classic() +
  labs(title = "Figure 5. Distribution of Average Post-Election Homicide Rates by Municipal Party Alternations in Mexico (2001-2020)",
       x = "Average Post-Election Homicide Rate (Per 100,000 People)",
       y= "Frequency") +
  theme(plot.title = element_text(hjust = 0.5))

```

### Figure 6: PAN Power Index Histogram
...
ggplot(elections_hom, aes(x = post_election_avg_rate)) +
  ...
  theme_classic() +
  labs(title = "Figure 6. Distribution of Average Post-Election Homicide Rates by PAN Consecutive Years in Power in Mexico (2001-2020)",
       x = "Average Post-Election Homicide Rate (Per 100,000 People)",
       y= "Frequency") +
  theme(plot.title = element_text(hjust = 0.5))
